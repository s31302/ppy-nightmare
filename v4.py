...
class PpyMareGame:
    ...
    def frames_maker(self):
        ...
        self.bStart = tk.Button(self.frames['frameMenu'], image=self.img_play, background="black", borderwidth=0, activebackground="black", command=lambda: self.show_temp_frame('frameStart', 'frameMap', 'bStart'))
        ...
        self.b1 = tk.Button(self.frames['frameMap'], image=self.img_game1, background="black", borderwidth=0, activebackground="black", command=lambda: self.show_temp_frame('frameStartStory1', 'frameFight1', 'b1'))
        self.b1.place(relx=0.17, rely=0.5, anchor="center")

        self.b2 = tk.Button(self.frames['frameMap'], image=self.img_game2, background="black", borderwidth=0, activebackground="black", command=lambda: self.show_temp_frame('frameStartStory2', 'frameFight2', 'b2'))
        self.b2.place(relx=0.33, rely=0.25, anchor="center")

        self.b3 = tk.Button(self.frames['frameMap'], image=self.img_game3, background="black", borderwidth=0, activebackground="black", command=lambda: self.show_temp_frame('frameStartStory3', 'frameFight3', 'b3'))
        self.b3.place(relx=0.50, rely=0.25, anchor="center")

        self.b4 = tk.Button(self.frames['frameMap'], image=self.img_game4, background="black", borderwidth=0, command=lambda: self.show_temp_frame('frameStartStory4', 'frameFight4', 'b4'))
        self.b4.place(relx=0.67, rely=0.5, anchor="center")

        self.b5 = tk.Button(self.frames['frameMap'], image=self.img_game5, background="black", borderwidth=0, command=lambda: self.show_temp_frame('frameStartStory5', 'frameFight5', 'b5'))
        self.b5.place(relx=0.83, rely=0.5, anchor="center")

        # przekazuje wlsciwosci z game 2 i 3 do 4 i 5
        self.b2d = tk.Button(self.frames['frameMap'], image=self.img_game2, background="black", borderwidth=0, activebackground="black", command=lambda: self.show_temp_frame('frameStartStory2', 'frameFight2', 'b2d'))
        self.b2d.place(relx=0.33, rely=0.75, anchor="center")

        self.b3d = tk.Button(self.frames['frameMap'], image=self.img_game3, background="black", borderwidth=0, activebackground="black", command=lambda: self.show_temp_frame('frameStartStory3', 'frameFight3', 'b3d'))
    ...
    def text_typer(self, label, text, i=0, delay=75):
        if not label.winfo_exists():
            self.typing_after_id = None
            # Jeśli etykieta nie istnieje, zatrzymaj odtwarzanie dźwięku pisania
            if self.chan_type.get_busy():
                self.chan_type.fadeout(100)  # Szybkie wyciszenie
            return
    ...
    def start_text_typer(self, label, text, delay=75):
        ...
        if self.typing_after_id:
            # Sprawdzamy czy etykieta, której dotyczył poprzedni 'after' wciąż istnieje
            # Jeśli nie, to after_cancel może wywołać błąd.
            # Bardziej robustne podejście to przechowywanie ID w słowniku
            # powiązanym z etykietami, ale dla jednego globalnego ID wystarczy
            try:
                label.after_cancel(self.typing_after_id)
            except tk.TclError:  # Możliwe, że już został zniszczony, więc ignorujemy błąd
                pass
            self.typing_after_id = None
    ...
    def show_frame(self, name):
        ...
        if hasattr(self, 'animation_id') and self.animation_id:
            try:
                ...
                pass  # Ignoruj, jeśli już zostało zniszczone

            self.animation_id = None
            # Stop typing sound if still playing when switching frames
        if self.chan_type.get_busy():
            self.chan_type.fadeout(500)

        if hasattr(self, 'typing_after_id') and self.typing_after_id:
            try:
                # Użyj label.after_cancel(self.typing_after_id) jeśli self.typing_after_id jest z label.after
                # W twoim przypadku jest z label.after, więc musisz mieć dostęp do tej etykiety.
                # Jeśli show_frame nie ma dostępu do konkretnej etykiety, to trudniej to anulować z tego miejsca.
                # Rozwiązanie: zamiast globalnego self.typing_after_id, każda ramka mogłaby mieć swój after_id,
                # albo text_typer powinien być bardziej samoobsługowy.
                # Na potrzeby tej poprawki, zakładam, że self.typing_after_id jest globalne.
                # Najlepiej to anulować w text_typer albo w miejscu, gdzie wywołujesz start_text_typer dla poprzedniej ramki.
                # Tutaj możemy jedynie spróbować anulować, jeśli wiemy, który label był ostatnio użyty.
                pass  # Pozostawiamy to do rozwiązania w start_text_typer lub w logice show_frame jeśli jest bardziej złożona
            except tk.TclError:
                pass
            self.typing_after_id = None

            # Jeśli przechodzisz do ramki walki, upewnij się, że self.attack_outcome i self.mercy_outcome są zresetowane
            # Zmieniłem nazwy dla większej czytelności i by uniknąć kolizji z atrybutami klasy.
            # Użyj self.attack_outcome i self.mercy_outcome zamiast self.attack i self.mercy w logice walki.
        self.attack_outcome = False
        self.mercy_outcome = False
        ...
        if name == "frameFight1":
            ...
            # For the final boss, determine the ending before starting the fight
            self.current_ending = self.which_ending()  # Update the instance variable
            self.fight_frame(enteties.fish_boss, name, self.current_ending, self.fish_boss_image1,
                             self.fish_boss_image2, self.fish_sound)
        elif name == "frameLose":  # Specific handling for the lose frame
            self.chan_bg.play(self.death_sound)  # Play death sound
            self.chan_bg.fadeout(1500)
            # The game will transition to 'frameMenu' after 'frameLose' is displayed for a short time
            self.after_id = self.root.after(4000, lambda: self.show_frame(
                'frameMenu'))  # Display lose for 3 seconds then go to menu
            # Additionally, reset player stats when they lose
            enteties.player.reset()
            self.attack_outcome = False  # Reset outcome for new game
            self.mercy_outcome = False  # Reset outcome for new game
            self.switch("lose")  # Reset map buttons
            # Ensure the story label for 'frameLose' is updated if it exists
            #if 'frameLose' in self.story_labels:
            #    self.story_labels['frameLose'].config(text=self.story_texts['frameLose'])
            if name in self.story_labels and self.story_labels[name].winfo_exists():
                self.story_labels[name].config(text=self.story_texts[name])
        elif name == "frameMenu":
            self.chan_bg.play(self.sound_bg, loops=-1)  # Restart background music for menu
            self.chan_bg.set_volume(0.3)
            # When returning to menu, ensure all buttons on the map are reset for a new game
            self.switch("lose")
        ...
        if name in self.story_texts:
            ...
            # Sprawdź, czy etykieta, którą zamierzamy użyć, faktycznie istnieje
            if lbl.winfo_exists():
                self.start_text_typer(lbl, self.story_texts[name])
            else:
                # Jeśli etykieta nie istnieje (np. ramka jest świeżo tworzona i nie ma jeszcze labela),
                # możesz tu dodać logikę do utworzenia labela lub wyświetlić błąd.
                # Na razie, po prostu zresetuj ID, by uniknąć wiszących odwołań.
                self.typing_after_id = None
    ...
    def fight_frame(self, enemy: Enemy, start_frame, end_frame, photo1, photo2, music):
        ...
        # labels (pozostają bez zmian, bo są zawsze widoczne)
        ...
        self.animation_id = None  # Dodaj to do klasy, aby móc anulować animację

        def animation(frame1, frame2):
            if self.root.winfo_exists() and image_label.winfo_exists():  # Dodatkowo sprawdzamy, czy label istnieje
                ...
                # Jeśli okno root lub label nie istnieje, nie planuj więcej wywołań
                self.animation_id = None

        animation(photo1, photo2)

        # funkcje (pozostają bez zmian, ale będą używać nowych przycisków/etykiet)
        ...
        # Utwórz WSZYSTKIE widżety potrzebne w tej ramce na początku
        # Przyciski główne walki
        ...
        # Widżety inwentarza (początkowo ukryte)
        info_label = tk.Label(self.frames[start_frame], bg="black", fg="white", font=(50))

        EggCheckbutton_var = tk.IntVar()  # Zmieniono nazwę, aby nie kolidowała z funkcją
        ...
        # Funkcje pomocnicze dla inwentarza (teraz operują na istniejących widżetach)
        def config_info():
        ...
        def inventory_clicked():
            # Ukryj przyciski główne walki
            fight_button.place_forget()
            mercy_button.place_forget()
            inventory_button.place_forget()

            # Pokaż przyciski inwentarza i info_label
            config_info()  # Ustaw tekst info_label zanim go pokażesz
        ...
        def exit_inventory():
            # Ukryj przyciski inwentarza i info_label
        ...
        # Początkowo pokaż tylko główne przyciski walki
        fight_button.place(relx=0.25, rely=0.9, anchor="center")
        mercy_button.place(relx=0.50, rely=0.9, anchor="center")
        inventory_button.place(relx=0.75, rely=0.9, anchor="center")
    ...
    # Funkcja which_ending pozostaje bez zmian
