...
class PpyMareGame:
    def __init__(self, root):
        ...
        # Inicjalizacja dźwięków z obsługą błędów (bez użycia os)
        # Jeśli plik dźwiękowy nie zostanie znaleziony, użyjemy pustego obiektu Sound
        self.sound_bg = self._load_sound('sounds/C418 - Wet Hands - Minecraft Volume Alpha.mp3')
        self.sound_type = self._load_sound('sounds/typing-sound-effect.mp3')
        self.spider_sound = self._load_sound('sounds/Pokemon Black & White Music_ Driftveil City Music.mp3')
        self.water_blob_sound = self._load_sound('sounds/The Trapper.mp3')
        self.bimbo_sound = self._load_sound('sounds/You Have to Cut the Dope (mp3cut.net).mp3')
        self.dog_sound = self._load_sound('sounds/In Game (From Granny).mp3')
        self.fish_sound = self._load_sound('sounds/Remnants in Dreams (mp3cut.net).mp3')
        self.mix_ending_sound = self._load_sound(
            'sounds/Bleach Ost Soundscape to ardor (Morning Remembrance) [AMV].mp3')
        self.bad_ending_sound = self._load_sound('sounds/Little Nightmares II End of the Hall.mp3')
        self.good_ending_sound = self._load_sound('sounds/Welcome to Moominvalley.mp3')
        self.death_sound = self._load_sound('sounds/Hades - Death and I (mp3cut.net).mp3')
        ...
        # after_id będzie przechowywać ID dla głównego timera (np. animacji walki, przejścia między ramkami)
        self.after_id = None
        # typing_after_id będzie przechowywać ID dla timera efektu pisania tekstu
        self.typing_after_id = None

        # Ładowanie obrazów z obsługą błędów (bez użycia os)
        # Jeśli plik obrazu nie zostanie znaleziony, zostanie utworzony placeholder w pamięci
        self.img_skip = self._load_image('images/skip2.png', "white")

        # Odtwarzanie muzyki w tle, jeśli dźwięk został poprawnie załadowany
        if self.sound_bg:
            self.chan_bg.play(self.sound_bg, loops=-1)
            self.chan_bg.set_volume(0.3)
        else:
            print(
                "Ostrzeżenie: Dźwięk tła nie został załadowany. Sprawdź plik 'sounds/C418 - Wet Hands - Minecraft Volume Alpha.mp3'.")

        self.typing_after_id = None

        self.ending = ""

        # asety zosi - ładowanie czcionki z obsługą błędów
        pyglet.options['win32_gdi_font'] = True
        self.fontpath = Path(__file__).parent / 'assets/Ghastly Panic.ttf'
        try:
            pyglet.font.add_file(str(self.fontpath))
            self.scary_font = pyglet.font.load("Ghastly Panic").name
        except Exception as e:
            print(
                f"Ostrzeżenie: Nie udało się załadować czcionki 'Ghastly Panic.ttf': {e}. Używam domyślnej czcionki 'Helvetica'.")
            self.scary_font = "Helvetica"  # Fallback font

        # Ładowanie obrazów postaci z obsługą błędów
        self.spider_image1 = self._load_image("images/spider_1-removebg-preview.png", "green")
        self.spider_image2 = self._load_image("images/spider_2-removebg-preview.png", "lightgreen")
        self.water_blob_image1 = self._load_image("images/blob_2-removebg-preview.png", "cyan")
        self.water_blob_image2 = self._load_image("images/blob_1-removebg-preview.png", "lightblue")
        self.bimbo_bear_image1 = self._load_image("images/bimbo_2-removebg-preview.png", "pink")
        self.bimbo_bear_image2 = self._load_image("images/bimbo_1-removebg-preview.png", "lightpink")
        self.talking_dog_image1 = self._load_image("images/dog_1-removebg-preview.png", "brown")
        self.talking_dog_image2 = self._load_image("images/dog_2-removebg-preview.png", "darkbrown")
        self.fish_boss_image1 = self._load_image("images/fish_1-removebg-preview.png", "orange")
        self.fish_boss_image2 = self._load_image("images/fish_2-removebg-preview.png", "gold")

        self.frames_maker()

        self.show_frame('frameMenu')

        # Powiązanie funkcji on_closing z protokołem zamknięcia okna
        # Dzięki temu wszystkie timery zostaną bezpiecznie zatrzymane przed zamknięciem okna
        self.root.protocol("WM_DELETE_WINDOW", self.on_closing)

    def _load_image(self, path, placeholder_color):
        """Pomocnicza funkcja do ładowania obrazów lub tworzenia placeholdera."""
        try:
            return tk.PhotoImage(file=path)
        except tk.TclError:
            print(f"Ostrzeżenie: Nie udało się załadować obrazu '{path}'. Tworzę placeholder.")
            img = tk.PhotoImage(width=100, height=100)
            # Użyj 'put' do wypełnienia placeholdera kolorem
            img.put(placeholder_color, to=(0, 0, 99, 99))
            return img

    def _load_sound(self, path):
        """Pomocnicza funkcja do ładowania dźwięków lub zwracania None."""
        try:
            return pygame.mixer.Sound(path)
        except pygame.error as e:
            print(f"Ostrzeżenie: Nie udało się załadować dźwięku '{path}': {e}. Dźwięk będzie pomijany.")
            # Zwróć bardzo krótki, cichy Sound, aby uniknąć błędów podczas play()
            # Alternatywnie można zwrócić None i dodać if sound_object: before play()
    ...
    def frames_maker(self):
        ...
        for frame in (
        'frameMenu', 'frameStart', 'frameMap', 'frameStartStory1', 'frameStartStory2', 'frameStartStory3', 'fight',
        'frameStartStory4', 'frameStartStory5', 'frameFight1', 'frameFight2', 'frameFight3', 'frameFight4',
        'frameFight5',
        'frameEndStory1', 'frameEndStory2', 'frameEndStory3', 'frameEndStory4', 'mixed_ending', 'bad_ending',
        'good_ending', 'frameWin', 'frameLose', 'frameSpare'):
            f = tk.Frame(container, bg="black")
            f.grid(row=0, column=0, sticky="nsew")
            self.frames[frame] = f

        self.bStart = tk.Button(self.frames['frameMenu'], text="Play",
                                command=lambda: self.show_temp_frame('frameStart', 'frameMap', 'bStart'))
        self.bStart.place(relx=0.5, rely=0.5, anchor="center")

        tk.Label(self.frames['frameMenu'], text="title", fg="white", bg="black", font=("Helvetica", 30), ).place(
            relx=0.5, rely=0.25, anchor="center")

        self.b1 = tk.Button(self.frames['frameMap'], image=self.img_skip, background="black", borderwidth=0,
                            activebackground="black",
                            command=lambda: self.show_temp_frame('frameStartStory1', 'frameFight1', 'b1'))
        self.b1.place(relx=0.17, rely=0.5, anchor="center")

        self.b2 = tk.Button(self.frames['frameMap'], image=self.img_skip, background="black", borderwidth=0,
                            activebackground="black",
                            command=lambda: self.show_temp_frame('frameStartStory2', 'frameFight2', 'b2'))
        self.b2.place(relx=0.33, rely=0.25, anchor="center")

        self.b3 = tk.Button(self.frames['frameMap'], text="Game 3",
                            command=lambda: self.show_temp_frame('frameStartStory3', 'frameFight3', 'b3'))
        self.b3.place(relx=0.50, rely=0.25, anchor="center")

        self.b4 = tk.Button(self.frames['frameMap'], text="Game 6",
                            command=lambda: self.show_temp_frame('frameStartStory4', 'frameFight4', 'b4'))
        self.b4.place(relx=0.67, rely=0.5, anchor="center")

        self.b5 = tk.Button(self.frames['frameMap'], text="Game 7",
                            command=lambda: self.show_temp_frame('frameStartStory5', 'frameFight5', 'b5'))
        self.b5.place(relx=0.83, rely=0.5, anchor="center")

        # przekazuje wlsciwosci z game 2 i 3 do 4 i 5
        self.b2d = tk.Button(self.frames['frameMap'], text="Game 4",
                             command=lambda: self.show_temp_frame('frameStartStory2', 'frameFight2', 'b2d'))
        self.b2d.place(relx=0.33, rely=0.75, anchor="center")

        self.b3d = tk.Button(self.frames['frameMap'], text="Game 5",
                             command=lambda: self.show_temp_frame('frameStartStory3', 'frameFight3', 'b3d'))
        self.b3d.place(relx=0.50, rely=0.75, anchor="center")

        self.story_texts = {
            'frameStart': (
                "A wet feeling on your face wakes you up. It's rain. You look around. You don’t know where you are but everything looks like from a dream. It seems like you’re in the middle of a forest. You’re laying in moss but you don’t remember how you got here. Something blinking in the distance caught your attention. You come closer to it and it turns out it’s in a cave. Curiosity gets the better of you and you enter. "),
            'frameStartStory1': (
                "Upon entering you notice a figure. It looks like a spider. Maybe it will tell you more? Intrigued, you come closer."),
            'frameStartStory2': (
                "You land on ground and it's a miracle that you're alright. You look around. You're on a small Island. It's very warm and pleasent. Then you notice that you didn't get hurt because you fell on a weird watery blob."),
            'frameStartStory3': (
                "Suddenly, (you don't know how), you're in school. A wave of memories floods you. Old anxiety and sweat on your hands returns. In front of you appeares that one popular girl who laughed at you. The way she's loudly chewing gum drives you crazy."),
            'frameStartStory4': (
                'You want to move on to look for exit but there is a dog standing in front of you. "You smell like bones…"  it says (which surprises you). "Share them with me. I am H U N G R Y".'),
            'frameStartStory5': (
                'There is emptiness, only a huge fish with legs. "I have been watching you all along. I am the one who shapes reality, your confusion pleases me. I am not letting you go."'),
            'frameEndStory1': (
                "You decide to leave the cave, but where the forest used to be, there's nothing left. Not even ground. Then you fall."),
            'frameEndStory2': ("A bird grabs you and takes you somewhere far away."),
            'frameEndStory3': (
                "You hear a bell and you wake up in your bed. Phew thankfully it was just an alarm that you haven't turned off since high school. You check the clock but can't see clearly what time is it. It's probably because you're still sleepy. You go to the bathroom to wash your face. You look up at the mirror, but it isn't your face that you see. The staring person from the mirror strats coming out and subconsciously you know that it doesn't mean anything good. You grab the doorknob and run out of the bathroom straight into the hotel corridor. You run and run until you're out of breath. You look back expecting to see THAT THING, but corridor is empty. Phew it seems like you managed to escape. "),
            'frameEndStory4': (
                "You move on, but the corridors don't end. You turn again and you see the same painting, the same plant on right and the same (qustionable) stain on the carpet, God only knows what it's from... You look up and an the end of the hallway you see your old, lost love. It's that person, the one you spent your wonderful vacations with. You feel disoriented and in a rush of adrenaline you start running desperately towards her, shouting her name, but she disapperes behind the door. You run up there and enter the room and..."),
            'mixed_ending': ("Weird fish vanished. You decide to look for exit once more. You have to get out."),
            'bad_ending': (
                "All of this is too much for you. You decide to get out. It doesn't matter who else you have to kill to leave this place."),
        ...
        for key in self.story_texts.keys():
            if key != 'frameWin' and key != 'frameLose' and key != 'frameSpare':
                lbl = tk.Label(self.frames[key], text="", fg="white", bg="black", font=("Helvetica", 30),
                               wraplength=1000)
    ...
    def switch(self, button_name):
        match button_name:
            ...
            case "lose":
                # Odtwarzanie dźwięku w tle, jeśli został poprawnie załadowany
                if self.sound_bg:
                    self.chan_bg.play(self.sound_bg, loops=-1)
    ...
    def text_typer(self, label, text, i=0, delay=75):
        if i == 0:
            ...
            # Odtwarzanie dźwięku pisania, jeśli został poprawnie załadowany
            if not self.chan_type.get_busy() and self.sound_type:
        ...
        if (i < len(text)):
            ...
            # Tekst napisany, zatrzymaj muzykę
            if self.chan_type.get_busy():
                self.chan_type.fadeout(800)
            self.typing_after_id = None  # Resetuj ID po zakończeniu pisania
            return
    ...
    def show_frame(self, name):
        # Stop any currently running animation or delayed action before showing a new frame
        self.stop_all_timers()  # Użyj nowej funkcji do zatrzymywania wszystkich timerów
        ...
        match name:
            ...
            case "frameFight2":
                self.fight_frame(enteties.water_blob, name, "frameEndStory2", self.water_blob_image1,
                                 self.water_blob_image2, self.water_blob_sound)
            case "frameFight3":
                self.fight_frame(enteties.bimbo_bear, name, "frameEndStory3", self.bimbo_bear_image1,
                                 self.bimbo_bear_image2, self.bimbo_sound)
            case "frameFight4":
                self.fight_frame(enteties.talking_dog, name, "frameEndStory4", self.talking_dog_image1,
                                 self.talking_dog_image2, self.dog_sound)
            case "frameFight5":
                self.fight_frame(enteties.fish_boss, name, "frameEndStory5", self.fish_boss_image1,
                                 self.fish_boss_image2, self.fish_sound)
    ...
    def show_temp_frame(self, temp_frame, return_frame, button_name=None):
        ...
        # Planowanie powrotu do ramki docelowej
        # Zapisujemy ID, aby móc je później anulować (np. przy zamknięciu okna)
    ...
    def fight_frame(self, enemy: Enemy, start_frame, end_frame, photo1, photo2, music):
        # Odtwarzanie muzyki walki, jeśli została poprawnie załadowana
        if music:
        ...
        # current_frame dla animacji tej ramki walki
        self.current_frame = 0;

        # Definicja funkcji animacji (wewnętrzna funkcja ma dostęp do photo1, photo2, image_label)
        def local_animation_loop():
            ...
            # Planowanie następnego wywołania animacji i zapisywanie ID
            self.after_id = self.root.after(500, local_animation_loop)

        # Uruchomienie animacji. Ważne: stop_all_timers() już został wywołany w show_frame()
        # więc nie musimy tutaj ponownie anulować self.after_id.
        local_animation_loop()
        ...
        def player_enemy_fight():
            ...
            if enteties.player.health_points <= 0:
                # Odtwarzanie dźwięku śmierci, jeśli został poprawnie załadowany
                if self.death_sound:
        ...
        def inventory_clicked():
            ...
            EggButton = tk.Checkbutton(self.frames[start_frame], text="           Egg          ",
                                       variable=EggCheckbutton,
                                       bg="black",
                                       fg="white", font=(self.scary_font, 30),
                                       command=lambda: (
                                       check_button(EggCheckbutton, "egg"), config_info(), config_stats()))
            ...
            LeafButton = tk.Checkbutton(self.frames[start_frame], text="           Leaf          ",
                                        ...
                                        fg="white", font=(self.scary_font, 30),
                                        command=lambda: (
                                        check_button(LeafCheckbutton, "leaf"), config_info(), config_stats()))
            ...
            FeatherButton = tk.Checkbutton(self.frames[start_frame], text="         Feather        ",
                                           ...
                                           fg="white", font=(self.scary_font, 30),
                                           command=lambda: (
                                           check_button(FeatherCheckbutton, "feather"), config_info(), config_stats()))
    ...
    def stop_all_timers(self):
        """Metoda zatrzymująca wszystkie aktywne timery Tkinter."""
        if self.after_id:
    ...
    def on_closing(self):
        """Metoda wywoływana przy próbie zamknięcia okna. Zatrzymuje timery i niszczy okno."""
        print("Zamykanie okna, zatrzymywanie wszystkich timerów...")
        self.stop_all_timers()  # Zatrzymujemy wszystkie timery
        self.root.destroy()  # Niszczymy okno Tkinter
...
if __name__ == "__main__":
    # Usuwamy wszelkie operacje na plikach związane z modułem 'os' z tego bloku.
    # Teraz ładowanie/tworzenie placeholderów odbywa się w metodach klasy App.
    root = tk.Tk()
