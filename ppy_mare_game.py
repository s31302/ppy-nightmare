...
class PpyMareGame:
    def __init__(self, root):
        ...
        self.sound_bg = pygame.mixer.Sound('C418 - Wet Hands - Minecraft Volume Alpha.mp3')
        self.sound_type = pygame.mixer.Sound('typing-sound-effect.mp3')  # podaj ścieżkę do muzyki
    ...
    def frames_maker(self):
        ...
        tk.Button(self.frames['frameMap'], text="Game 2", command=lambda: self.show_temp_frame('frameGame2', 'frameMap')).place(relx=0.33, rely=0.25, anchor="center")
        ...
        #wypsiywanie proba
        labelHistoryGame1 = tk.Label(self.frames['frameGame1'], text="", fg="white", bg="black", font=("Helvetica", 30), wraplength=1000)
        labelHistoryGame1.place(x=0, y=0, relwidth=1, relheight=1)
        tekst = ("Na wejściu widzisz postać. Przypomina (wklej opis postaci). Może ona ci powie coś więcej? Zaintrygowana podchodzisz bliżej ")
        self.text_typer(labelHistoryGame1, tekst)

        tk.Button(self.frames['frameGame1'], image=self.img_skip, borderwidth=0, background='black',
                  activebackground='black', command=lambda: self.show_frame('frameMap')).place(relx=0.87, rely=0.80)
